#!/bin/bash
#
# Simple bash script to run all tests in parallel on Ubuntu with ASAN
# and UBSAN enabled.
#
...
#
#   run_test() { time ./ovs_unittests.py "${@:2}" --sanitize asan ubsan --ubuntu -p --vagrant-vm-cpus=4 --vagrant-vm-name=ubuntu-$1 -r $1; }
#   run_test XX_testcase_XX
#

export RUN_TEST="run_test() { sleep \$((RANDOM % 10)); time ./ovs_unittests.py \$2 $@ --sanitize asan ubsan --ubuntu --vagrant-vm-cpus=4 --vagrant-vm-name=ubuntu-\$1 -r \$1; }"

tmux new -s ovs_ubuntu_tests            \; \
